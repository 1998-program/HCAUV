...
void AP_Motors6DOF::setup_motors(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    case SUB_FRAME_CUSTOM:
        add_motor_raw_6dof(AP_MOTORS_MOT_1,     1.0f,           -1.0f,          0,              -1.0f,              0,                  0,              1);
        add_motor_raw_6dof(AP_MOTORS_MOT_2,     -1.0f,          -1.0f,          0,              -1.0f,              0,                  0,              2);
        add_motor_raw_6dof(AP_MOTORS_MOT_3,     1.0f,           1.0f,           0,              -1.0f,              0,                  0,              3);
        add_motor_raw_6dof(AP_MOTORS_MOT_4,     -1.0f,          1.0f,           0,              -1.0f,              0,                  0,              4);
        add_motor_raw_6dof(AP_MOTORS_MOT_5,     0,              0,              1.0f,           0,                  -1.0f,              1.0f,           5);
        add_motor_raw_6dof(AP_MOTORS_MOT_6,     0,              0,              -1.0f,          0,                  -1.0f,              -1.0f,          6);
        add_motor_raw_6dof(AP_MOTORS_MOT_7,     0,              0,              -1.0f,          0,                  1.0f,               1.0f,           7);
        add_motor_raw_6dof(AP_MOTORS_MOT_8,     0,              0,              1.0f,           0,                  1.0f,               -1.0f,          8);        
            ...
            for (i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
                if (motor_enabled[i]) {
                    if(_yaw_factor[i] > 0){
                        rpy_out[i] = roll_thrust * _roll_factor[i] +
                                    pitch_thrust * _pitch_factor[i] +
                                    yaw_thrust * _yaw_factor[i] * 0.5625;
                    }
                    else{
                        rpy_out[i] = roll_thrust * _roll_factor[i] +
                                    pitch_thrust * _pitch_factor[i] +
                                    yaw_thrust * _yaw_factor[i] * 0.4375;
                    }
                }
            }
